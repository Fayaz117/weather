...
def fetch_weather_details(city_name):
    url = f"http://api.openweathermap.org/data/2.5/weather?q={city_name}&appid={openweathermap_api_key}"
...
def get_openweathermap_current_weather(city):
    url = f"http://api.openweathermap.org/data/2.5/weather?q={urllib.parse.quote(city)}&appid={openweathermap_api_key}&units=metric"
    response = requests.get(url)
...
def main():
    ...
    if st.button("Predictions For Next 3 Days"):
        visual_crossing_url = "https://weather.visualcrossing.com/VisualCrossingWebServices/rest/services/timeline/goa%2C%20india/last15days?unitGroup=metric&include=days&key=M8Y7MK6356T28MAQHA7E8KWWU&contentType=csv"
        response = requests.get(visual_crossing_url)

        if response.status_code == 200:
            csv_data = response.text
            df = pd.read_csv(StringIO(csv_data))
            target_variables = ['temp', 'feelslike', 'humidity',
                                'windspeed', 'cloudcover', 'precip', 'dew', 'solarradiation']
            X = df[['tempmin', 'tempmax', 'feelslikemin', 'feelslikemax', 'humidity',
                    'windspeed', 'cloudcover', 'precip', 'dew', 'solarradiation']]
            ...
            for day in range(2, num_days + 1):
                formatted_date = (dates.iloc[-1] + pd.DateOffset(days=day)).strftime("%Y-%m-%d")
